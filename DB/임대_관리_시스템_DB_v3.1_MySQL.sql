...
CREATE TABLE `center` (
    `center_id`         INT           NOT NULL AUTO_INCREMENT COMMENT '센터 고유 식별자',
...
CREATE TABLE `branch` (
    `branch_id`      INT           NOT NULL AUTO_INCREMENT COMMENT '지점 고유 식별자',
    `branch_name`    VARCHAR(100)  NOT NULL COMMENT '지점명',
    `status_center`  CHAR(1)       NOT NULL DEFAULT 'N' COMMENT '지점 상태 (Y: 활성, N: 비활성)',
    `address`        TEXT          NULL     COMMENT '주소',
    ...
    CONSTRAINT `chk_branch_status_center` CHECK (`status_center` IN ('Y', 'N')),
...
CREATE TABLE `manager` (
    `manager_id` INT           NOT NULL AUTO_INCREMENT COMMENT '담당자 식별자',
    `branch_id`  INT           NOT NULL COMMENT '소속 지점 식별자',
    `name`       VARCHAR(50)   NOT NULL COMMENT '담당자명',
    `contact`    VARCHAR(15)   NULL     COMMENT '연락처',
    `email`      VARCHAR(100)  NOT NULL COMMENT '이메일',
    `is_main`    CHAR(1)       NOT NULL DEFAULT 'N' COMMENT '주/부 담당자 여부 (Y/N)',
    `created_at` DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '생성 시점',
    `deleted_at` DATETIME      NULL     COMMENT '삭제 시점',
    PRIMARY KEY (`manager_id`),
    CONSTRAINT `chk_manager_is_main` CHECK (`is_main` IN ('Y', 'N'))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='지점 담당자';

-- 5. 디바이스 모델 테이블
DROP TABLE IF EXISTS `device_model`;
CREATE TABLE `device_model` (
    `model_id`   INT           NOT NULL AUTO_INCREMENT COMMENT '모델 고유 식별자',
...
CREATE TABLE `device` (
    `device_id`        INT           NOT NULL AUTO_INCREMENT COMMENT '디바이스 고유 ID',
    `model_id`         INT           NOT NULL COMMENT '모델 식별자',
    `branch_id`        INT           NOT NULL COMMENT '현재 배정된 지점 식별자',
...
CREATE TABLE `rental` (
    `rental_id`        INT           NOT NULL AUTO_INCREMENT COMMENT '임대 이력 식별자',
    `device_id`        INT           NOT NULL COMMENT '대여된 디바이스 ID',
    `branch_id`        INT           NOT NULL COMMENT '임대 지점',
...
CREATE TABLE `device_as` (
    `as_id`            INT           NOT NULL AUTO_INCREMENT COMMENT 'AS 접수 식별자',
    `device_id`        INT           NOT NULL COMMENT 'AS 대상 디바이스 ID',
    `branch_id`        INT           NOT NULL COMMENT 'AS 발생 지점 식별자',
    ...
    `user_id`          INT           NOT NULL COMMENT '접수자 ID',
    `rental_id`        INT           NULL     COMMENT '임대 이력 식별자',
...
CREATE TABLE `emergency` (
    `emergency_id`       INT           NOT NULL AUTO_INCREMENT COMMENT '응급 접수 식별자',
    `type_emergency`     VARCHAR(50)   NULL     COMMENT '응급 유형 (과호흡, 낙상 등)',
    `user_id`            INT           NOT NULL COMMENT '사용자 ID',
    `device_id`          INT           NOT NULL COMMENT '디바이스 고유 ID',
    `event_time`         DATETIME      NULL     COMMENT '응급 상황 발생 시각',
    `location_address`   TEXT          NULL     COMMENT '응급 상황 발생 주소',
    `status_emergency`   TINYINT       NOT NULL DEFAULT 0 COMMENT '응급 상황 처리 상태 (대기:0, 오탐:1, 처리완료:2) — 등록 시 DEFAULT 0',
...
CREATE TABLE `identifier_master` (
    `code_id`           INT           NOT NULL AUTO_INCREMENT COMMENT '코드 마스터 식별자',
    `type_entity`       TINYINT       NOT NULL DEFAULT 0 COMMENT '엔티티의 유형(ex: USER, DEVICE)',
    `entity_pk_value`   INT           NULL     COMMENT '실제 엔티티 테이블의 기본 키 값',
...
-- 12. 디바이스 사용 생체 정보 로그 테이블
DROP TABLE IF EXISTS `device_log`;
CREATE TABLE `device_log` (
    `log_id`             BIGINT        NOT NULL AUTO_INCREMENT COMMENT '생체 데이터 로그 식별자',
    `device_id`          INT           NOT NULL COMMENT '데이터를 수집한 디바이스 ID',
...
ALTER TABLE `device_as`
    ...
    -- [오타 수정] 기존 'usTABLE_CONSTRAINTSer'를 'user'로 수정 완료
